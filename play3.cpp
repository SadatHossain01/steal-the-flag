...
struct Game {
        ...
        if (n_minions == -1) {
            ...
            for (int idx = 0; idx < n_minions - 2; idx++) {
                ...
                for (int now = start; now <= end; now++) {
                    if (not_visited[now] != make_pair(myFlagBaseX, myFlagBaseY))
    ...
    void doThings(const Game& last, const Game& last2) {
        ...
        for (int j = 0; j < n_minions; j++) {
            ...
            if (!firstTime) {
                for (int j = 0; j < n_minions; j++) {
                    if (!last.isOppVisible[j] || !isOppVisible[j]) continue;
            ...
            if (!nullify) {
                ...
                for (int dir = 0; dir < 4; dir++) {
                    ...
                    for (int j = 0; j < n_minions && ok; j++) {
                        ...
                        //     if (!askSameLine(oppMinions[j].posX, oppMinions[j].posY, oppFlagX,
                        //     oppFlagY, false, -1, last)) continue;
                        // }
