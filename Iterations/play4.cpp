...
inline bool sameLine(int x1, int y1, int x2, int y2, bool wantAdjAnalysis, int lastX, int lastY) {
    cerr << "x1: " << x1 << " y1: " << y1 << " x2: " << x2 << " y2: " << y2 << " " << lastX << " "
         << lastY << endl;
    //  x2 y2 is the target
    bool ret = false;
    int now = -1;
...
struct Game {
    ...
    bool askSameLine(int x1, int y1, int x2, int y2, bool wantAdjAnalysis, int targetID,
        ...
        else return sameLine(x1, y1, x2, y2, false, lastX, lastY);
    }
            ...
            if (i == mandatoryCarrier) {
                ...
                if (oppFlagCarrier == mandatoryCarrier) {
                    ...
                    if (!moveDone && distFromMyBase[xx][yy] <= 10 &&
                        myScore >= powerups[1].price)  // so close, so just freeze?
                    ...
                    if (!moveDone && (myMinions[i].health <= 40 &&
                                      myScore >= powerups[0].price))  // if you are down on health,
                ...
                if (myFlagCarrier == -1) {
                    ...
                    if (myScore >= powerups[0].price) {
                        if (!moveDone &&
                            ...
                            if (d.first == 0) takeOperation(FIRE, i, moveDone);
                    ...
                    if (!not_visited.empty()) {
                        if (*(explorerMinions.begin()) == i) {
                            ...
                            while (!leftToVisit[now.first][now.second]) {
                                ...
                                if (!not_visited.empty()) now = not_visited.front();
                                else break;
                            }
                            ...
                            while (!leftToVisit[now.first][now.second]) {
                                ...
                                if (!not_visited.empty()) now = not_visited.back();
                                else break;
                            }
                            if (!not_visited.empty()) dest = now;
                        }
